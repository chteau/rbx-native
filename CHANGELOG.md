...
- **A union drawn as its recovered pieces is patched piece by piece, not
  ...
  leaves it, and only an edit pointing one at an asset nobody has fetched
  is still a reload (`Rebuild::Asset`). Three reasons for a rebuild are
  left where there were four. The union itself stays one thing to the
