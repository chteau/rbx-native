...
| Dimensions | `topbar_height()` 34 · `menu_bar_height()` 30 · `tabs_height()` 42 · `ribbon_tabs_height()` 28 · `ribbon_height()` 80 · `dock_tabs_height()` 38 · `dock_width()` 300 · `row_height()` 28 · `row_label_width()` 120 · `input_height()` 31 · `checkbox_size()` 15 in a `checkbox_target()` of 26 · … | **functions**, all scaled. Larger than the frame's own numbers because the type is (see below) |
