...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    /// Every selection's byte range, the active (primary) one first.
    ///
    /// rbx-native addition, alongside [`Self::add_selection`].
    pub fn selected_ranges(&self) -> Vec<Range<usize>> {
        self.selections.iter().map(|sel| (*sel).into()).collect()
    }

    /// Adds `range` as a secondary selection, keeping every existing one —
    /// the building block for "add cursor to next matching selection". A
    /// range overlapping an existing selection merges into it. No-op on a
    /// single-line input.
    ///
    /// rbx-native addition: upstream keeps the selection list private.
