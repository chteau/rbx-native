...
fn a_non_zero_rotation_makes_the_elements_own_clips_descendants_a_no_op() {
    // Roblox's own docs for `ClipsDescendants` are explicit that it is
    // ignored wherever this element or an ancestor has a non-zero
    // `Rotation`, since a scissor rect can't follow a rotated box.
    let (mut dom, gui) = screen_gui();
    let rotated = frame(
        &mut dom,
        gui,
