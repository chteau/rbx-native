...
//! [`properties`] for what is read out of a DOM in the first place, and
//! [`constraints`] for the modifiers that change how big a box is before any
//! of that: `UIPadding`, `UIScale`, the `UIConstraint` family, `AutomaticSize`
//! and `SizeConstraint`, and [`placement`] for the rest of what decides where
//! a box lands — nested rotation, `BorderMode`, `ScreenInsets` and
//! `ZIndexBehavior`.
//!
