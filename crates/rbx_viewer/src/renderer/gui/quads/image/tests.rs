...
use super::super::shape::{FILL, UNBOUNDED};
use super::*;
use crate::scene::GuiJoin;

/// A white, opaque, unshaped paint: what every image test draws with, since
/// only the UVs and positions are under test here.
fn plain() -> (Paint<'static>, Shape) {
    (
        Paint {
            color: [1.0, 1.0, 1.0],
            alpha: 1.0,
...
fn slice_borders_grow_with_slice_scale() {
    ...
    let (paint, shape) = plain();
    slice(
        &rect(0.0, 0.0, 200.0, 150.0),
        [100.0, 100.0],
        Some(center),
        2.0,
        (&paint, &shape, &Spin::new(0.0, [100.0, 75.0])),
        &mut vertices,
    );
...
fn slice_falls_back_to_a_single_quad_for_a_zero_sized_texture() {
    ...
    slice(
        ...
        None,
        1.0,
        (&paint, &shape, &Spin::new(0.0, [25.0, 25.0])),
        &mut vertices,
    );
