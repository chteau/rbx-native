...
use super::shape::{quad, Paint, Shape, Spin};
...
pub(super) fn build(
    ...
    match &image.scale {
        GuiImageScale::Stretch => {
            ...
            quad(rect, (uv0, uv1), paint, shape, spin, into);
        }
        ...
        GuiImageScale::Tile => {
            quad(rect, ([0.0, 0.0], image.repeat), paint, shape, spin, into);
        }
        GuiImageScale::Fit => {
            let (drawn, uv0, uv1) = fit(rect, image, texture_size);
            quad(&drawn, (uv0, uv1), paint, shape, spin, into);
        }
        GuiImageScale::Crop => {
...
fn slice(
    ...
    if tex_w <= 0.0 || tex_h <= 0.0 {
        quad(rect, super::shape::UV_WHOLE, paint, shape, spin, into);
        return;
    }
