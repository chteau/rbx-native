...
/// outermost `Model` around whatever it hit ([`outermost_model`], matching
/// Studio), so most selections a user makes by clicking arrive here as a
/// container rather than as a part.
///
...
/// (`hits`, nearest first — see [`parts_along`]) and whatever
/// is selected now.
///
...
pub fn outermost_model(dom: &WeakDom, database: &ReflectionDatabase, referent: Ref) -> Ref {
